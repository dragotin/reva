...
func (fs *posixfs) Upload(ctx context.Context, req storage.UploadRequest, uff storage.UploadFinishedFunc) (provider.ResourceInfo, error) {
	...
	if err != nil {
		...
		if upload, err = fs.GetUpload(ctx, uploadIDs["simple"]); err != nil {
			return provider.ResourceInfo{}, errors.Wrap(err, "posixfs: error retrieving upload")
		}
	}
	...
	ok := chunking.IsChunked(p) // check chunking v1
	if ok {
		...
		if err != nil {
			return provider.ResourceInfo{}, errors.Wrap(err, "posixfs: error opening assembled file")
		}
		defer fd.Close()
	...
	if _, err := uploadInfo.WriteChunk(ctx, 0, req.Body); err != nil {
		return provider.ResourceInfo{}, errors.Wrap(err, "posixfs: error writing to binary file")
	}

	return provider.ResourceInfo{}, uploadInfo.FinishUpload(ctx)
}
